...
if __name__ == '__main__':
    ...
    # ===========================================
    fig, ax = plt.subplots()
    sns.kdeplot(np.array(prior_costs)/1e6, label='Prior', ax=ax, c='k', alpha=0.5, lw=2, cut=0)
    sns.kdeplot(np.array(posterior_costs)/1e6, label='Pre-Posterior', ax=ax, c='k', lw=2, cut=0)
    ymax = ax.get_ylim()[1]
    ...
    fig, ax = plt.subplots()
    sns.kdeplot(prior_lcoes, label='Prior', ax=ax, c='k', alpha=0.5, lw=2, cut=0)
    sns.kdeplot(posterior_lcoes, label='Pre-Posterior', ax=ax, c='k', lw=2, cut=0)
    ymax = ax.get_ylim()[1]
