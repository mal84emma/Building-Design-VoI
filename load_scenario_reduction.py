...
def get_scenario_stats(building_scenario_vector, load_profiles_dict):
    ...
    i.e. set of building-load_year profiles (profiles for each building in scenario)."""

    load_profiles = []

    for building_tuple in building_scenario_vector:
        solar_year = building_tuple[0]
        building_id,load_year = building_tuple[1:3]
        mean,peak = building_tuple[3:]

        load_profile = load_profiles_dict[f'{int(building_id)}-{int(load_year)}']

        # perform profile scaling
...
def reduce_load_scenarios(sampled_scenarios, load_profiles_dict, num_scenarios=10):
    ...
    Scenarios are described by vectors of building id-load_year pairs (tuple) for each building in the scenario."""
